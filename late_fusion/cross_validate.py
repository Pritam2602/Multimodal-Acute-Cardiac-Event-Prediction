...
def main():
    parser = argparse.ArgumentParser(description="Cross-validation for Late Fusion AMI model")
    ...
    parser.add_argument("--epochs", type=int, default=NUM_EPOCHS, help="Max epochs per fold")
    parser.add_argument("--learning-rate", type=float, default=LEARNING_RATE, help="Optimizer learning rate")
    parser.add_argument("--dropout", type=float, default=DROPOUT_RATE, help="Model dropout")
    parser.add_argument("--loss-name", choices=["focal", "bce"], default="focal", help="Loss function to use")
    parser.add_argument("--focal-alpha", type=float, default=FOCAL_LOSS_ALPHA, help="Alpha parameter for focal loss")
    parser.add_argument("--focal-gamma", type=float, default=FOCAL_LOSS_GAMMA, help="Gamma parameter for focal loss")
    parser.add_argument("--num-workers", type=int, default=None, help="Override training DataLoader worker count")
    parser.add_argument("--val-workers", type=int, default=VAL_NUM_WORKERS, help="Override validation DataLoader worker count")
    parser.add_argument("--weighted-sampling", action="store_true", help="Use a weighted sampler for training folds")
    parser.add_argument("--run-name", type=str, default="crossval", help="Artifacts subdirectory under artifacts/runs")
